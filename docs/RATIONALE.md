...
The defining hazard of a generated documentation site, and the reason this entry sits near the top
of what to know.
...
## 11. The changelog workflow: a shared concurrency group and a marker that must not spread

Two decisions in `changelog.yaml`, both of which look like cruft and are not.

**The concurrency group is shared with the strip pipeline on purpose.** Both push to the production
branch and would otherwise race. It must never use `cancel-in-progress` — a dropped run loses a
content revision permanently, and nothing reports the loss.

**Never put a skip-CI marker on the development-side commit.** The production-side commit needs it
to stop the workflow retriggering itself. Copying it to the other side prevents nothing — the
workflow does not trigger on that branch anyway — and silently disarms the quality gate on **every**
promotion opened from that commit.
