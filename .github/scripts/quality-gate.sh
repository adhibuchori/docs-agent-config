...
TODOS=$(git diff --name-only "$BASE"...HEAD -- 'content/technical/**/*.mdx' | xargs -r grep -lE '_TODO_|TODO: Add usage example' 2>/dev/null)
if [ -n "$TODOS" ]; then
  echo "::warning::Generated docs above still contain TODO placeholders — fill them in before merging."
fi
...
scan "Dangerous JS APIs Check" '\beval\s*\(|new\s+Function\s*\(' "scripts"
scan "Unsafe React Patterns Check" 'dangerouslySetInnerHTML|__html' "scripts"
scan "URL Scheme Injection Check" '(javascript:|data:text/html|data:application/)' "scripts"
...
# Both dirs: out/ ships, .next/ is what it copies from. See README § CI/CD.
