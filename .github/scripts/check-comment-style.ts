...
const KEYWORDS_BEFORE_REGEX = /\b(return|typeof|instanceof|in|of|new|delete|void|throw|case|do|else|yield|await)$/;
...
function findViolations(source: string): Violation[] {
  ...
  while (i < n) {
    ...
    if (c === '/') {
      ...
      const isRegex = trimmed === '' || !VALUE_CANNOT_FOLLOW.test(trimmed) || KEYWORDS_BEFORE_REGEX.test(trimmed);
      if (isRegex) {
        i++;
        let inClass = false;
        while (i < n) {
