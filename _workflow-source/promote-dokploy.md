...
**This repo's deploy target:** Cloudflare Workers, deployed by `cloudflare/wrangler-action` in `ci-cd.yaml`
...
### 3.1 Deploy directly to Cloudflare

```bash
bun run deploy        # wrangler deploy
```

This is the same publish `cloudflare/wrangler-action` performs in `ci-cd.yaml`. It needs a
Cloudflare API token in your environment (`CLOUDFLARE_API_TOKEN`), the same secret CI uses.

Build first if the deploy does not build for you — a stale `.next`/`dist` publishes silently.
...
The newest entry must be dated after the push in 2.2. Then load the site and confirm the
content actually changed — a successful upload of an unchanged bundle looks identical.

### 3.3 Smoke test

Load the published site and exercise the change itself, not just that the process is up.
